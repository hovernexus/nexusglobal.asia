...
function viewEquipmentDetails(equipment) {
    ...
    // V14.0: Navigate to the product detail page using the equipment ID
    window.location.href = `product-detail.html?id=${equipment.id}`;
    
// Fallback: Show equipment details in a modal if no detailUrl is provided
// This provides better UX and avoids creating separate detail pages. However, for now, we prioritize direct navigation.
// If a detailUrl is not explicitly provided, we construct one based on the equipment ID.
    
    // Create detail modal HTML
    // If for some reason the equipment.id is not suitable for direct navigation,
    // or if the product-detail.html page expects a different parameter,
    // this fallback modal could be re-enabled or adjusted.
    // For now, we assume direct navigation to product-detail.html?id=... is the intended behavior.

    // Original modal fallback code (commented out for direct navigation focus):
    /*
