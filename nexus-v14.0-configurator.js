...
function createEquipmentCard(equipment, rank) {
    ...
    card.innerHTML = `
        ...
        <div class="card-actions">
            ...
            <button class="card-btn recommendation-btn-secondary view-details-btn" onclick="viewEquipmentDetails(currentRecommendations.find(e => e.id === '${equipment.id}'))">
                View Details
            </button>
        </div>
    `;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // V14.0: Ensure quote functions are initialized and viewEquipmentDetails is available
    if (typeof initializeQuoteFunctions === 'function') {
        initializeQuoteFunctions();
        console.log('Quote functions initialized');
    }

    // The view-details-btn now directly calls viewEquipmentDetails via inline onclick attribute
    // The previous event listener for dynamically created 'View Details' buttons is no longer needed here
    // as the onclick is directly embedded in the button creation.
});
